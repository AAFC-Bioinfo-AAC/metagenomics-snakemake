...
- **MAGs from individual samples**: Each sample is assembled using *MEGAHIT*. The resulting assemblies are indexed, and the reads are mapped back with *Bowtie2* and *Samtools*. *MetaBAT* bins the assembled contigs into putative genomes, which are evaluated for completeness and contamination using *CheckM2*. The checkpoint in this module uses values specified in the config file to filter samples that meet the minimum required number of contigs and base pairs for inclusion in the remaining steps.
- **CAZyme annotation**: This module follows the [run_dbCAN](https://run-dbcan.readthedocs.io/en/latest/index.html) workflow for CAZyme and CAZyme Gene Cluster (CGC) annotation, and then uses *dbcan_utils* to generate gene depth files and calculate CAZyme abundances in Reads Per Million (RPM). A checkpoint occurs prior to the dbCAN analysis and ensures that only samples that successfully produced an assembly are included in downstream CAZyme, CGC, and substrate analyses.
