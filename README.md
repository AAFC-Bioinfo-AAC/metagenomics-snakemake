...
The metagenomic-snakemake workflow was designed for paired reads from high biomass host-associated metagenomic samples that were sequenced on a Illumina platform. The workflow is modular and modules can be omitted as long as the required input files are present. The pre-processing module uses `fastp` for quality control, trimming and filtering paired reads. Using `Bowtie2` and `Samtools`, the trimmed reads are aligned to the host and PhiX genome and the reads that do not align are retained. The host and PhiX decontaminated reads are used as input for the taxonomy, antimicrobial resistance, and functional pathways module. If the user is not interested in one of the modules it can easily be removed from the pipeline without affecting the other two modules. The taxonomy module uses `Kraken2` and `Bracken` to produce abundance tables containing all the samples from the short reads at the species, genus and phylum level. The `Kraken` formatted database includes host taxonomy and reads matching to the host are removed prior to calculating the relative abundance. The user guide explains how to change the host parameters. Next, the antimicrobial resistance module uses `RGI` (resistance gene identifier) to map the short reads to CARD (Comprehensive Antibiotic Resistance Database). This produces a report containing the potential antibiotic resistance genes for each sample. The third module is the functional pathways and it produces an abundance table with high-level functional pathways for each sample. The concatenated decontaminated reads are used as input for `DIAMOND` alignment to the KEGG (Kyoto Encyclopedia of Genes and Genomes) protein database. The output of this is used to generate a KO (KEGG Orthology) table, which with some formatting  is used as input for `MinPath`. `Minpath` is used to deduce the minimum set of pathways that describe the set of genes. Using the `Minpath` confirmed pathways, the pathways are summarized into high-level categories with KEGG BRITE hierarchy. The module that assembles the reads from individual samples is the MAGs (metagenome assembled genomes) module. It uses `MEGAHIT` to assemble the individual samples then `Bowtie2` and `Samtools` to index the assembly then map the individual sample reads back to the assembly. `MetaBAT` is used to bin assembled contigs into potential genomes that are checked for completeness and contamination with `CheckM2`.
...
> 👉 *REQUIRED: If your project does not use the default MIT license provided here, replace the LICENSE file accordingly. Be sure to include the copyright statement both in this section (for visibility) and within the LICENSE file. Helpful resources to understand and select the right license include [LicenseHub](https://licensehub.org) (see their [Smart License Recommendation](https://licensehub.org/en/tools/selector) and [License Template Generator](https://licensehub.org/en/tools/generator) tools) and [tl;drLegal](https://www.tldrlegal.com/). Github provides a way to [add a license directly into the repository](https://docs.github.com/en/communities/setting-up-your-project-for-healthy-contributions/adding-a-license-to-a-repository).*
